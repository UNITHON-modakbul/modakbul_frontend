...
`master` 브랜치에 변경사항이 푸시되면 GitHub Actions가 프론트엔드를 빌드하고 `dist/` 결과물을 `s3://mvpilot.cloud`에 동기화합니다. CloudFront가 이 버킷을 원본으로 사용하며, 배포 ID가 설정되어 있으면 업로드 후 캐시 무효화도 요청합니다. AWS 인증은 장기 액세스 키 대신 GitHub OIDC와 IAM Role을 사용합니다.
...
워크플로는 기존 CloudFront 배포 설정을 임의로 변경하지 않습니다. 배포 결과 업로드와 캐시 무효화만 담당합니다.
...
| `CLOUDFRONT_DISTRIBUTION_ID` | 권장 | 업로드 후 캐시를 무효화할 CloudFront 배포 ID |
| `SITE_URL` | 선택 | Actions 실행 결과에 표시할 웹 사이트 URL |
| `VITE_API_BASE_URL` | 선택 | 프로덕션 API 기본 주소 |
| `VITE_SERVICE_PREVIEW_URL` | 선택 | 프로덕션 서비스 미리보기 주소 |

배포 버킷은 요청된 `mvpilot.cloud`로 워크플로에 고정되어 있습니다. CloudFront 배포 ID가 없으면 S3 업로드는 정상 진행되고 캐시 무효화 단계만 건너뜁니다.
...
HTML은 캐시하지 않고, 나머지 정적 파일은 1시간 캐시합니다. S3에는 `dist/`와 동일하지 않은 이전 파일을 자동으로 삭제합니다. `CLOUDFRONT_DISTRIBUTION_ID`가 설정되어 있으면 배포 직후 `/*` 무효화를 요청합니다.
