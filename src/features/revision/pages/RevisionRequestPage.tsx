    ...
    <section className="grid min-h-0 flex-1 place-items-center py-6">
      <div className="w-full max-w-2xl rounded-[30px] border border-[#17332f]/15 bg-white/92 p-7 text-center shadow-[0_24px_70px_rgba(23,51,47,0.12)] sm:p-10">
        ...
        <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[#17332f]/55">
          바로 수정하기 전에 기존 서비스와 요청 내용을 비교해 빠진 결정이 없는지 확인합니다.
        </p>
    ...
    <section className="mx-auto flex min-h-0 w-full max-w-4xl flex-1 flex-col py-4">
      ...
      <div className="flex shrink-0 items-center justify-between gap-3 border-t border-[#17332f]/10 pt-3">
        ...
        <div className="ml-auto grid w-full grid-cols-2 gap-3 sm:flex sm:w-auto">
          <Button disabled={isFirst || isSubmitting} onClick={onBack} type="button" variant="outline">
            <ArrowLeft aria-hidden="true" size={17} />
            이전 질문
          </Button>
          <Button disabled={!isAnswered || isSubmitting} onClick={onNext} type="button">
            {isSubmitting ? (
              <LoaderCircle aria-hidden="true" className="animate-spin" size={17} />
            ) : isLast ? (
              <Send aria-hidden="true" size={17} />
            ) : null}
            {isSubmitting ? '답변 전달 중' : isLast ? '수정 요청 확정' : '다음 질문'}
            {!isLast && !isSubmitting && <ArrowRight aria-hidden="true" size={17} />}
          </Button>
        </div>
      </div>
    </section>
...
export function RevisionRequestPage() {
  ...
  const { projectId = 'demoforge-pipeline' } = useParams()
  ...
  const canSubmit = target.trim().length >= 2 && requestedChange.trim().length >= 10
  ...
  const handleNextQuestion = async () => {
    ...
    if (!isRequirementAnswered(activeQuestion, answers[activeQuestion.id])) return
      ...
      <main className="relative z-10 mx-auto flex h-[calc(100dvh-4rem)] w-full max-w-7xl px-4 pb-4 sm:h-[calc(100dvh-72px)] sm:px-8 sm:pb-6 lg:px-10">
        {validateRevision.isPending ? (
          ...
          <RevisionQuestionFlow
            ...
            onBack={() => setActiveQuestionIndex((current) => Math.max(0, current - 1))}
          ...
          <section className="grid min-h-0 flex-1 place-items-center py-6">
            <div className="w-full max-w-2xl rounded-[30px] border border-[#17332f]/15 bg-white/92 p-8 text-center shadow-[0_24px_70px_rgba(23,51,47,0.12)] sm:p-11">
              ...
              <p className="mt-7 text-xs font-black tracking-[0.14em] text-[#5f8a39]">REVISION READY</p>
              <h1 className="mt-3 text-3xl font-black tracking-[-0.05em] sm:text-4xl">수정 방향이 확정됐어요.</h1>
              <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[#17332f]/55">
                입력한 요청과 후속 답변을 AI 개발 작업으로 전달했습니다. 수정이 완료되면 새로운 미리보기를 확인할 수 있어요.
              </p>
              <div className="mx-auto mt-7 flex max-w-md flex-col-reverse gap-3 sm:flex-row">
                <Button className="flex-1" onClick={() => navigate('/')} type="button" variant="outline">
                  <LayoutDashboard aria-hidden="true" size={16} />
                  대시보드
                </Button>
                <Button className="flex-1" onClick={() => navigate(previewPath)} type="button">
                  미리보기로 돌아가기
          ...
          <section className="grid min-h-0 flex-1 items-center gap-5 py-5 lg:grid-cols-[0.8fr_1.2fr]">
            <div className="hidden px-4 lg:block">
              ...
              <p className="mt-6 text-xs font-black tracking-[0.14em] text-[#ec6b42]">REQUEST A REVISION</p>
              <h1 className="mt-3 text-4xl font-black leading-tight tracking-[-0.055em]">어디를 어떻게<br />바꾸고 싶나요?</h1>
              <p className="mt-4 max-w-sm text-sm leading-6 text-[#17332f]/55">
                AI가 현재 결과와 수정 요청을 비교한 뒤, 모호하거나 충돌하는 부분만 후속 질문으로 확인해요.
              </p>
              <div className="mt-6 space-y-3">
                {['수정 대상 지정', 'AI 방향 검증', '후속 질문 후 재개발'].map((item, index) => (
                  <p className="flex items-center gap-3 text-xs font-bold text-[#375226]" key={item}>
                    <span className="grid size-6 place-items-center rounded-full bg-[#e9f2cc] font-mono text-[10px] font-black">{index + 1}</span>
                    {item}
                  </p>
                ))}
              </div>
            ...
            <div className="rounded-[30px] border border-[#17332f]/15 bg-white/92 p-6 shadow-[0_24px_70px_rgba(23,51,47,0.12)] sm:p-8">
              <div className="flex items-center gap-3 border-b border-[#17332f]/10 pb-5">
                ...
                <div>
                  <h2 className="text-lg font-black tracking-[-0.035em]">수정 요청 작성</h2>
                  <p className="text-xs text-[#17332f]/45">기능 변경이 아니라 원하는 결과를 자연어로 알려주세요.</p>
                </div>
              </div>

              <div className="mt-5">
                <label className="text-sm font-black" htmlFor="revision-target">수정할 화면 또는 영역</label>
                <Input
                  className="mt-2"
                  id="revision-target"
                  maxLength={80}
              ...
              <div className="mt-5">
                <div className="flex items-center justify-between gap-3">
                  <label className="text-sm font-black" htmlFor="revision-request">원하는 수정 방향</label>
                  <span className="font-mono text-[10px] text-[#17332f]/35">{requestedChange.length}/500</span>
                </div>
                <Textarea
                  className="mt-2 h-36"
                  id="revision-request"
              ...
              {validateRevision.isError && (
                <p className="mt-3 text-xs font-bold text-[#b94727]" role="alert">수정 방향 검증에 실패했어요. 잠시 후 다시 시도해주세요.</p>
              )}

              <div className="mt-6 grid grid-cols-[auto_1fr] gap-3">
                <Button onClick={() => navigate(previewPath)} type="button" variant="outline">
                  <ArrowLeft aria-hidden="true" size={17} />
                  미리보기
                </Button>
                <Button disabled={!canSubmit} onClick={handleValidate} type="button">
